...
class MaxPooling:
    def __init__(self, kernel_size=2, stride=2, padding=0):
        """
        初始化最大池化层
        :param kernel_size: 池化核的大小（默认为 2x2）
    ...
    def forward(self, x):
        ...
        col = col.reshape(-1, self.kernel_size * self.kernel_size, C).transpose(0, 2, 1)
    ...
    def backward(self, dout):
        ...
        # 初始化梯度矩阵
        col_dx = np.zeros((dout.shape[0], C, self.kernel_size * self.kernel_size), dtype=np.float32)

        # 矢量化梯度分配
        rows = np.arange(dout.size // C).repeat(C)
        channels = np.tile(np.arange(C), dout.size // C)
        col_dx[rows, channels, self.arg_max.ravel()] = dout.ravel()

        # 调整维度顺序以匹配 col2im 的输入格式，变为 (N * out_h * out_w, filter_size * filter_size * C)
        col_dx = col_dx.transpose(0, 2, 1).reshape(-1, C * self.kernel_size * self.kernel_size)
        dx = col2im(col_dx, self.input_shape, self.kernel_size, self.kernel_size, self.stride, self.padding)
